...
pub struct TileList {
    ...
    /// Next node ID
    next_node_id: Rc<RefCell<TileId>>,
...
impl TileList {
    pub fn new(layer_list: LayerList, dimension: usize) -> Self {
        Self {
            ...
            layers: HashMap::new(),
            arena: HashMap::new(),
            next_node_id: Rc::new(RefCell::new(TileId::new(0))),
    ...
    pub fn generate(&mut self) {
        ...
        let mut layer_list_borrow = self.layer_list.layers.borrow(); // Borrow once

        // iterate each layer
        for layer_id in self.layer_list.layer_ids.borrow().iter() {
            ...
            let Some(layer) = layer_list_borrow.get(&layer_id) else {
                continue;
            };
